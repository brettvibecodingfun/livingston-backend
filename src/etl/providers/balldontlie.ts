 ...
 */
export async function fetchTeamSeasonAverages(
  season: number,
  seasonType: string = 'regular',
  teamIds?: number[]
): Promise<ApiTeamSeasonAverage[]> {
  ...
  if (!teamIds || teamIds.length === 0) {
    ...
    const response = await fetchFromAPI(
      ...
      z.object({ data: z.array(TeamSeasonAverageSchema) }),
      params
    );
  ...
  const allResults: ApiTeamSeasonAverage[] = [];
 ...
 * Filters by team_ids array (only current NBA teams: IDs 1-30)
 */
export async function fetchAdvancedTeamSeasonAverages(
  season: number,
  seasonType: string = 'regular',
  teamIds?: number[]
): Promise<ApiTeamSeasonAverage[]> {
