 ...
 * Batches player_ids into chunks of 25 to avoid URL length limits
 */
export async function fetchClutchSeasonAverages(season, seasonType = 'regular', playerIds) {
    ...
    if (!playerIds || playerIds.length === 0) {
        ...
        const response = await fetchFromAPI('/season_averages/clutch', z.object({ data: z.array(SeasonAverageSchema) }), params);
        return response.data.filter(avg => avg.player?.id !== undefined && avg.player?.id !== null);
    }
    // Batch player IDs into chunks of 25 to avoid URL length limits
    const BATCH_SIZE = 25;
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        const response = await fetchFromAPI('/season_averages/clutch', z.object({ data: z.array(SeasonAverageSchema) }), batchParams);
        // Filter to only include entries with player.id and season (required for our use case)
        const validResults = response.data.filter(avg => avg.player?.id !== undefined &&
            avg.player?.id !== null &&
            avg.season !== undefined &&
 ...
 * Batches player_ids into chunks of 25 to avoid URL length limits
 */
export async function fetchAdvancedClutchSeasonAverages(season, seasonType = 'regular', playerIds) {
    ...
    if (!playerIds || playerIds.length === 0) {
        ...
        const response = await fetchFromAPI('/season_averages/clutch', z.object({ data: z.array(AdvancedSeasonAverageSchema) }), params);
        return response.data.filter(avg => avg.player?.id !== undefined && avg.player?.id !== null);
    }
    // Batch player IDs into chunks of 25 to avoid URL length limits
    const BATCH_SIZE = 25;
    ...
    for (let i = 0; i < batches.length; i++) {
        ...
        const response = await fetchFromAPI('/season_averages/clutch', z.object({ data: z.array(AdvancedSeasonAverageSchema) }), batchParams);
        // Filter to only include entries with player.id and season (required for our use case)
        const validResults = response.data.filter(avg => avg.player?.id !== undefined &&
            avg.player?.id !== null &&
            avg.season !== undefined &&
